...
def parse_binds(bind_args, newroot):
    ...
    for bindval in bind_args:
        ...
        if dest in invalid_dest:
            die(f"Error. Bind destination '{dest}' is already managed by mchroot automatically. ('--bind={bindval}')")
        binds[dest] = src
    return binds
...
def parse_args():
    ...
    ap.add_argument('--version', action='version', version=f"mchroot {__version__} ({__codename__})")
    ...
    ap.add_argument("--nobind", action="store_true", default=False, help="Disable all bind mounts when entering mchroot")
